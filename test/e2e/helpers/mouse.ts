...
export async function rightClickMenu(
  ...
  for (let attempt = 0; attempt < 3; attempt++) {
    try {
      ...
      // Let the menu's entry animation settle before measuring the click
      // point: coordinates taken mid-animation can land outside the moving
      // content, which Radix treats as an outside click — the menu closes
      // without selecting.
