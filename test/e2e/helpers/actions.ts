...
/**
 * Drive the goal card's context menu until the focus state actually flips —
 * a menu can dismiss without selecting when a click lands on animating
 * content, so the interaction is verified against the manualFocuses table
 * (via the auto bridge) and retried.
 */
async function setGoalFocusVia(
  page: Page,
  goalId: string,
  menuItemTestId: string,
  wantFocused: boolean
): Promise<void> {
  const hasFocusRow = async () =>
    (await auto(page).snapshot()).manualFocuses.some(
      (f: { goalId: string }) => f.goalId === goalId
    )
  for (let attempt = 0; attempt < 3; attempt++) {
    await rightClickMenu(page, goalCard(page, goalId), menuItemTestId)
    const deadline = Date.now() + 2000
    while (Date.now() < deadline) {
      if ((await hasFocusRow()) === wantFocused) return
      await page.waitForTimeout(150)
    }
  }
  throw new Error(
    `goal ${wantFocused ? "focus" : "unfocus"} did not take effect after 3 attempts`
  )
}
...
export async function focusGoal(page: Page, goalId: string): Promise<void> {
  await setGoalFocusVia(page, goalId, "ctx-goal-focus", true)
}

/** Right-click a goal card → Unfocus. */
export async function unfocusGoal(page: Page, goalId: string): Promise<void> {
  await setGoalFocusVia(page, goalId, "ctx-goal-unfocus", false)
}
...
export async function createTaskViaUI(
  ...
  // Picking a goal mounts the repeat section and grows the dialog, which can
  // move the button between measuring its center and clicking — a missed
  // click leaves the dialog open. Click until it actually closes.
  const titleField = page.locator('[data-testid="create-task-title"]')
  for (let attempt = 0; attempt < 3; attempt++) {
    if ((await submit.count()) === 0) break // already submitted, closing
    await click(page, submit)
    try {
      await expect(titleField).toHaveCount(0, { timeout: 2000 })
      return
    } catch {
      // re-click
    }
  }
  await expect(titleField).toHaveCount(0, { timeout: 5000 })
}
